...
class ASPP(nn.Module):
    def __init__(self, C, depth, num_classes, norm=nn.BatchNorm2d, momentum=0.0003, mult=1):
        ...
        self.aspp2 = nn.Conv2d(
            C, depth, kernel_size=3, stride=1, dilation=int(6 * mult), padding=int(6 * mult), bias=False
        )
...
class BottleneckX(nn.Module):
    ...
    def __init__(
        self, inplanes, planes, cardinality, stride=1, downsample=None, dilation=1, norm=None, elastic=False, se=False
    ):
...
class ResNext(nn.Module):
    ...
    def _make_mg(self, block, planes, dilation=2, multi_grid=(1, 2, 4), se=False):
        downsample = nn.Sequential(
            nn.Conv2d(self.inplanes, planes * block.expansion, kernel_size=1, stride=1, dilation=1, bias=False),
            self._norm(planes * block.expansion),
        )
