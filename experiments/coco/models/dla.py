...
class BottleneckXElastic(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, dilation=1):
        ...
        self.conv1_d = nn.Conv2d(inplanes, bottle_planes // 2, kernel_size=1, bias=False)
        ...
        self.conv3_d = nn.Conv2d(bottle_planes // 2, planes, kernel_size=1, bias=False)

        self.conv1 = nn.Conv2d(inplanes, bottle_planes // 2, kernel_size=1, bias=False)
        ...
        self.conv3 = nn.Conv2d(bottle_planes // 2, planes, kernel_size=1, bias=False)
...
class DLA(nn.Module):
    def __init__(
        ...
        self.level2 = Tree(
            levels[2], block, channels[1], channels[2], 2, level_root=False, root_residual=residual_root, seg=seg
        )
        self.level3 = Tree(
            levels[3], block, channels[2], channels[3], 2, level_root=True, root_residual=residual_root, seg=seg
        )
        self.level4 = Tree(
            levels[4], block, channels[3], channels[4], 2, level_root=True, root_residual=residual_root, seg=seg
        )
        self.level5 = Tree(
            levels[5], block, channels[4], channels[5], 2, level_root=True, root_residual=residual_root, seg=seg
        )
...
def dla60x_elastic(**kwargs):
    model = DLA([1, 1, 1, 2, 3, 1], [16, 32, 128, 256, 512, 1024], block=BottleneckXElastic, **kwargs)
    return model
...
def dla102x_elastic(**kwargs):
    ...
    model = DLA(
        [1, 1, 3, 3, 3, 1], [16, 32, 128, 256, 512, 1024], block=BottleneckXElastic, residual_root=True, **kwargs
    )
    return model
