...
import experiments.coco.models
import os
from PIL import Image
from experiments.coco.utils import add_flops_counting_methods, save_checkpoint, AverageMeter
...
parser.add_argument("-g", "--num-gpus", default=4, type=int, metavar="N", help="number of GPUs to match (default: 4)")
parser.add_argument("--lr", "--learning-rate", default=0.001, type=float, metavar="LR", help="initial learning rate")
parser.add_argument("--momentum", default=0.9, type=float, metavar="M", help="momentum")
parser.add_argument(
    "--weight-decay", "--wd", default=5e-4, type=float, metavar="W", help="weight decay (default: 5e-4)"
)
parser.add_argument("--print-freq", "-p", default=117, type=int, metavar="N", help="print frequency (default: 117)")
...
def main():
    ...
    if args.resume:
        if os.path.isfile(args.resume):
            ...
            resume = (
                "module.fc.bias" in checkpoint["state_dict"]
                and checkpoint["state_dict"]["module.fc.bias"].size() == model.module.fc.bias.size()
            ) or (
                "module.classifier.bias" in checkpoint["state_dict"]
                and checkpoint["state_dict"]["module.classifier.bias"].size() == model.module.classifier.bias.size()
            )
            if resume:
            ...
            print(
                "=> loaded checkpoint '{}' (epoch {})".format(
                    args.resume, checkpoint["epoch"] if "epoch" in checkpoint else "unknown"
                )
            )
        else:
...
def train_multi(train_loader, model, criterion, optimizer, epoch, iteration_size):
    ...
    for i, (_, input, target) in enumerate(train_loader):
        ...
        if i % args.print_freq == 0:
            ...
            print(
                "P_C {:.2f} R_C {:.2f} F_C {:.2f} P_O {:.2f} R_O {:.2f} F_O {:.2f}".format(
    ...
    for i, (_, input, target) in enumerate(val_loader):
        ...
        if i % args.print_freq == 0:
            print(
                ...
                "Precision {prec.val:.2f} ({prec.avg:.2f})\t"
                "Recall {rec.val:.2f} ({rec.avg:.2f})".format(
                    i, len(val_loader), batch_time=batch_time, loss=losses, prec=prec, rec=rec
                )
            )
            print(
                "P_C {:.2f} R_C {:.2f} F_C {:.2f} P_O {:.2f} R_O {:.2f} F_O {:.2f}".format(
